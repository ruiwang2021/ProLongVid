...
def process_video_with_decord(video_file, data_args, reset_frames=None):
    ...
    if reset_frames is not None and reset_frames > 0:
        uniform_sampled_frames = np.linspace(0, total_frame_num - 1, reset_frames, dtype=int)
        frame_idx = uniform_sampled_frames.tolist()
    elif data_args.fixed_num_frames is not None and data_args.fixed_num_frames > 0:
        uniform_sampled_frames = np.linspace(0, total_frame_num - 1, data_args.fixed_num_frames, dtype=int)
        frame_idx = uniform_sampled_frames.tolist()
    elif data_args.frames_upbound > 0 or data_args.frames_lowerbound > 0:
        if data_args.frames_upbound > 0 and len(frame_idx) > data_args.frames_upbound:
            uniform_sampled_frames = np.linspace(0, total_frame_num - 1, data_args.frames_upbound, dtype=int)
            frame_idx = uniform_sampled_frames.tolist()
        elif data_args.frames_lowerbound > 0 and len(frame_idx) < data_args.frames_lowerbound:
            uniform_sampled_frames = np.linspace(0, total_frame_num - 1, data_args.frames_lowerbound, dtype=int)
            frame_idx = uniform_sampled_frames.tolist()
