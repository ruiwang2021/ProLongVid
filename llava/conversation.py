...
class Conversation:
    ...
    def get_prompt(self):
        ...
        if self.sep_style == SeparatorStyle.SINGLE:
            ...
            if self.tokenizer is None:
                raise ValueError("Llama 3.1 tokenizer is not available. Make sure you have the necessary permissions.")
            chat_template_messages = [{"role": "system", "content": self.system}]
            for role, message in messages:
                if message:
                    if type(message) is tuple:
            ...
            if messages[-1][0] == self.roles[1] and messages[-1][1] is None:
...
conv_llava_llama_3 = Conversation(
    ...
    tokenizer=safe_load_tokenizer("pretrained/Meta-Llama-3-8B-Instruct"),
    stop_token_ids=[128009],
)

conv_llava_prolong_llama_3 = Conversation(
    system="You are a helpful language and vision assistant. " "You are able to understand the visual content that the user provides, " "and assist the user with a variety of tasks using natural language.",
    roles=("user", "assistant"),
    version="llama_v3",
    messages=[],
    ...
    sep="<|eot_id|>",
    sep_style=SeparatorStyle.LLAMA_3,
    tokenizer_id="princeton-nlp/Llama-3-8B-ProLong-512k-Instruct",
    tokenizer=safe_load_tokenizer("pretrained/Llama-3-8B-ProLong-512k-Instruct"),
    stop_token_ids=[128009],
)
...
conv_llava_llama_3_1 = Conversation(
    ...
    tokenizer=safe_load_tokenizer("pretrained/Llama-3.1-8B-Instruct"),
...
conv_templates = {
    ...
    "prolong_llama_3": conv_llava_prolong_llama_3,
}
