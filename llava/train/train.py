...
class ModelArguments:
    ...
    enable_vision_tower_flash_attn: bool = field(default=False)
    ...
    mm_fixed_patch_length: Optional[int] = field(default=None)
...
class DataArguments:
    ...
    frames_lowerbound: Optional[int] = field(default=0)
    fixed_num_frames: Optional[int] = field(default=None)
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    lora_enable_vision_tower: bool = False
...
def find_all_lora_names(model, target_modules, enable_vision_tower=False):
    lora_module_names = set()
    multimodal_keywords = ["mm_projector", "vision_resampler"]
    if not enable_vision_tower:
...
class LazySupervisedDataset(Dataset):
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources[0]:
            ...
            if "num_frames" in self.list_data_dict[i]:
                reset_num_frames = self.list_data_dict[i]["num_frames"]
            else:
                reset_num_frames = None
            ...
            try:
                if "shareVideoGPTV" in video_file:
                    ...
                    num_frames_to_sample = max(total_frames, 64)
                    if self.data_args.fixed_num_frames is not None and self.data_args.fixed_num_frames > 0:
                        num_frames_to_sample = self.data_args.fixed_num_frames
                    elif self.data_args.frames_upbound > 0 or self.data_args.frames_lowerbound > 0:
                        if self.data_args.frames_upbound > 0 and total_frames > self.data_args.frames_upbound:
                            num_frames_to_sample = self.data_args.frames_upbound
                        elif self.data_args.frames_lowerbound > 0 and total_frames < self.data_args.frames_lowerbound:
                            num_frames_to_sample = self.data_args.frames_lowerbound
                    ...
                    video, video_time, frame_time, num_frames_to_sample = process_video_with_decord(video_file, self.data_args, reset_num_frames)
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    if any(
        [
            ...
            model_args.enable_vision_tower_flash_attn,
        ]
    ):
    ...
    if model_args.enable_vision_tower_flash_attn:
        overwrite_config["enable_vision_tower_flash_attn"] = model_args.enable_vision_tower_flash_attn
...
def train(attn_implementation=None):
    ...
    if model_args.mm_spatial_pool_stride is not None:
        model.get_model().config.mm_spatial_pool_stride = model_args.mm_spatial_pool_stride
        model.config.mm_spatial_pool_stride = model_args.mm_spatial_pool_stride

    if model_args.mm_fixed_patch_length is not None:
        model.get_model().config.mm_fixed_patch_length = model_args.mm_fixed_patch_length
        model.config.mm_fixed_patch_length = model_args.mm_fixed_patch_length
    ...
    if model_args.vision_tower is not None:
        ...
        if training_args.lora_enable:
            ...
            lora_config = LoraConfig(
                r=training_args.lora_r,
                lora_alpha=training_args.lora_alpha,
                target_modules=find_all_linear_names(model) if training_args.lora_target_modules is None
                else find_all_lora_names(model, training_args.lora_target_modules, training_args.lora_enable_vision_tower),
                lora_dropout=training_args.lora_dropout,
                bias=training_args.lora_bias,
                task_type="CAUSAL_LM",
            )
            ...
            rank0_print(f"Trainable parameters' name: {trainable_params}")
